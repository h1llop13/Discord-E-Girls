...
async function generateExactCount(repository: CodeRepository, count: number): Promise<string[]> {
  const inserted: string[] = [];
  while (inserted.length < count) {
    const candidates = generateUniqueCodes(count - inserted.length);
    inserted.push(...(await repository.insertCodes(candidates)));
  }
  return inserted;
}
...
async function main(): Promise<void> {
  ...
  try {
    const codes = await generateExactCount(new CodeRepository(pool), count);
